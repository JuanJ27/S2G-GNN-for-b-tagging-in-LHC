...
def calc_metrics(pred_partitions, partitions_as_graph, partitions, accum_info):
    with torch.no_grad():
        ...
        accum_info['ri'] += (equiv_pairs.sum(dim=(1, 2)) / (N * (N - 1))).sum().item()
    return accum_info
...
def main():
    ...
    # Evaluate on test set with batching to avoid OOM
    ...
    test_info = do_epoch(test_data, best_model)
    print('Test results:')
    print(f"loss:{test_info['loss']:.6f} -- mean_ri:{test_info['ri']:.4f} -- fscore:{test_info['fscore']:.4f} -- recall:{test_info['recall']:.4f} -- precision:{test_info['precision']:.4f}")
    
    if config.save:
        test_results = pd.DataFrame(test_info, index=[0])

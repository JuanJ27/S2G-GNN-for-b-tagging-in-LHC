...
def transform_features(transform_list, arr):
    """
    Applies normalization to the features.
    """
...
class JetGraphDataset(Dataset):
    def __init__(self, which_set, debug_load=False, add_jet_flav=False, add_rave_file=False):
        ...
        :param debug_load: if True, loads only a small subset
        """
        ...
        if not torch.cuda.is_available():
            ...
            self.partitions = np.array([np.array(x) for x in self.partitions], dtype=object)
            self.partitions_as_graphs = np.array([np.array(x) for x in self.partitions_as_graphs], dtype=object)
    ...
    def get_all_items(self):
        """
        Generate all items for the dataset.
        """
...
class JetsBatchSampler(Sampler):
    def __init__(self, n_nodes_array, batch_size):
        ...
        for n_nodes_i in set(n_nodes_array):
            ...
            self.node_size_idx[n_nodes_i] = np.array_split(np.random.permutation(self.node_size_idx[n_nodes_i]), n_batches)
